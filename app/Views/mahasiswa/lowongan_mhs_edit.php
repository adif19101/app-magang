...
<script>
    $(document).ready(function() {
        ...
        if ($("#edit_lowongan_mhs").length > 0) {
            $("#edit_lowongan_mhs").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "<?= base_url('mahasiswa/lowongan/'). $lowongan['id'] ?>",
                        type: "POST",
                        data: $(form).serialize(),
                        headers: {
        ...
        $('#btn_save').on('click', function() {
            if ($('#edit_lowongan_mhs').valid()) {
                $('#edit_lowongan_mhs').submit();
            }
        });
    });
</script>
